...
app = FastAPI(title="Piracy Scanner", version="5.1")
...
def load_tabular_keywords():
    ...
    for path in paths:
        try:
            ...
            continue
        for col in ["title", "description", "name", "course", "keywords", "video_name", "video_title"]:
    ...
    push_log(f"📚 Loaded {len(KEYWORDS)} tokens, {len(KEYPHRASES)} phrases from tabular data")
...
def _build_google_queries() -> List[str]:
    base = ["pirated video", "education video", "medical video", "course leak", "telegram video"]
    fn_tokens = []
        ...
        try:
            ...
            for item in res.get("items", []):
                ...
                if sim >= SIM_THRESHOLD_PHOTO or kw > 0.7:
                    ...
                    push_log(f"✅ Google: {title[:80]} sim={sim:.2f} kw={kw:.2f}")
        except Exception as e:
    ...
    if getattr(msg, "photo", None):
        try:
            ...
            try:
                ...
                push_log(f"⚠️ TG {channel}: unsupported or corrupted photo format for msg {msg.id}")
                return
            embs = []
            for m in _models:
                try:
        ...
        try:
            ...
            frames_to_check = int(min(MAX_SAMPLE_SECONDS, max(duration_sec, MAX_SAMPLE_SECONDS)) * fps)
            ...
            try:
                ...
                push_log(f"⚠️ Couldn't delete temp video (in use): {tmp.name}")
...
async def scan_telegram(ref_embs: List[Any]) -> List[Dict[str, Any]]:
    ...
    seen = set()
    try:
        async for msg in client.iter_messages(None, search=base_q, limit=TG_GLOBAL_SEARCH_LIMIT):
            ...
            try:
                chat = getattr(msg, "chat", None)
                channel_name = getattr(chat, "title", "") or getattr(chat, "username", "") or "Global"
            except Exception:
                channel_name = "Global"
            await process_tg_msg(msg, ref_embs, results, channel_name, counters)
            if counters["media"] >= MAX_GLOBAL_MEDIA:
                break
    except Exception as e:
    ...
    try:
        async for dialog in client.iter_dialogs(limit=MAX_DIALOGS):
            channel_name = getattr(dialog.entity, "title", None) or getattr(dialog.entity, "username", None) or "Unknown"
            try:
                async for msg in client.iter_messages(dialog.entity, limit=MAX_MSGS_PER_DIALOG):
                    key = _safe_peer_key(msg)
                    if key in seen:
                        continue
                    seen.add(key)
...
async def upload_logos(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        try:
            ...
            img.load()  # load and verify integrity
        except (UnidentifiedImageError, OSError):
            return HTMLResponse(f"<h3>❌ Invalid or corrupted image: {file.filename}</h3>")
        REFERENCE_LOGO_NAMES.append(file.filename or "logo")
        logos_data.append({'embs': compute_embeddings_all_models(content), 'name': file.filename})
    try:
        LAST_RESULTS = await run_scan_multi(logos_data)
    except Exception as e:
        push_log(f"❌ Fatal scan error: {e}")
        return HTMLResponse("<h3>❌ Internal error during scan. Check /logs for details.</h3>", status_code=500)
...
async def results_page(request: Request):
    ...
    for r in LAST_RESULTS:
        ...
        matched_logos = ", ".join(r.get("matched_logo", [])) if isinstance(r.get("matched_logo", list)) else r.get("matched_logo", "")
        rows.append(
            f"<tr>"
            f"<td>{source}</td>"
            f"<td>{channel}</td>"
            ...
            f"<td>"
            f"<a href='/report?platform={source}&url={link}' target='_blank'>Report</a>"
            f"</td>"
            f"</tr>"
        )
    html = f"""
    <html><body style='font-family:Arial;padding:2rem'>
      ...
      <table border="1" cellspacing="0" cellpadding="6">
        <tr><th>Source</th><th>Channel</th><th>Item</th><th>Logo Sim</th><th>CSV Score</th><th>Matched Logo(s)</th><th>Action</th></tr>
...
async def download_csv():
    try:
        if not LAST_RESULTS:
            df = pd.DataFrame(columns=["source", "channel", "title", "link", "similarity", "kw_score", "score", "matched_logo"])
        else:
            normalized = []
            for r in LAST_RESULTS:
                normalized.append({
                    ...
                    "matched_logo": ", ".join(r.get("matched_logo", [])) if isinstance(r.get("matched_logo", list)) else r.get("matched_logo", ""),
                })
        ...
        return HTMLResponse("<h3>❌ Error generating CSV. Check /logs.</h3>", status_code=500)
...
async def report(request: Request, platform: str, url: str):
    ...
    if plat == "telegram":
        ...
        subject = "Unauthorized distribution of copyrighted educational content on Telegram"
...
Our content is identified by the logos: {', '.join(REFERENCE_LOGO_NAMES)}
...
[Contact details]
        ...
        subject = "DMCA / Unauthorized distribution of copyrighted content (Google Search result)"
        intro = f"For Google, you can either use their copyright form or email <b>{abuse_to}</b>."
        body = f"""
To: {abuse_to}
Subject: {subject}
...
[Contact details]
        ...
        intro = "Use the template below and send it to the abuse team of the platform."
        body = f"""
To: {abuse_to}
Subject: {subject}
